...
struct FinancialInstitutionCoordinator: MultipleChoiceViewModelCoordinator {
    ...
    func financialInstitutions()-> Promise<[ChoiceModel]> {
        ...
        return Promise<[ChoiceModel]>() { resolver in
            ...
            AuthConfig.shared.activeManager.getCurrentUser().then { authUser -> Promise<GetFinancialInstitutionResponse> in
                return CheqAPIManager.shared.getFinancesInstitutionsList()
            }.then { (getFinancialInstitutionResponse)->Promise<[ChoiceModel]>  in
                LoggingUtil.shared.cPrint("\n\n>>getFinancialInstitutionResponse = \(getFinancialInstitutionResponse)")
                LoggingUtil.shared.cPrint("\n\n>>institutions?.count = \(getFinancialInstitutionResponse.institutions?.count)")
                AppData.shared.resGetFinancialInstitutionResponse = getFinancialInstitutionResponse
                AppData.shared.financialInstitutions = getFinancialInstitutionResponse.institutions ?? []
                ...
                return Promise<[ChoiceModel]>() { res in
                    ...
                    for institution:GetFinancialInstitution in institutions {
                       let choice = ChoiceModel.init(type: .choiceWithIcon, title: institution.shortName ?? "N/A", caption: nil, image: institution.squareLogoUrl ?? "", ordering: institution.displayOrder!, ref: institution)
